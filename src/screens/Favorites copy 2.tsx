...
const Favorites: React.FC = () => {
      ...
      {favoriteItems.map((item, index) => (
        <Grid item xs={12} key={`${item.photoId}-${item.diptychIdCode}`} style={{ borderBottom: '1px solid #ccc', display: 'flex', alignItems: 'center', padding: '20px 0' }}>
          <Box display="flex" alignItems="center" style={{ width: '100%' }}>
            ...
            <Box flex="1" className={buttonStyles.clickable} onClick={() => handleFavoriteClick(item.photoId, item.diptychIdCode, item.seriesCode)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: `${diptychHeights[item.diptychIdCode] || 0}px` }}>
  ...
  const renderListView = () => (
    <Grid container spacing={2} style={{ marginTop: '20px' }}>
      {favoriteItems.map((item, index) => (
        <Grid item xs={12} key={`${item.photoId}-${item.diptychIdCode}`} style={{ borderBottom: '1px solid #ccc', display: 'flex', alignItems: 'center', padding: '10px 0' }}>
          <Box display="flex" alignItems="center" width="100%">
            <Box flex="0 0 200px" marginRight="20px" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '150px' }}>
              <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative' }}>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <DynamicDiptychComponent
                    photoId={item.photoId}
            ...
            <Box flex="1">
              ...
              <Box marginTop="10px">
                <button className={buttonStyles.button} onClick={() => navigate(`/${item.seriesCode}/${item.photoId}/inquire`)}>Inquire</button>
  ...
  return (
    <div>
      <Box position="sticky" top={60} bgcolor="white" zIndex={1000} paddingTop="20px" paddingBottom="10px">
